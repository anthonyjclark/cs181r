...
```{ojs}
div_id_obstacle = "jsxdiv-obstacle"

viewof graph_obstacle = html`<div id="${div_id_obstacle}" style="${graph_style}"></div>`
```

```{ojs}
import { WMRGraphObstacle } from '../_interactives/WMRGraph/dist/wmr-graph.js';
...
new WMRGraphObstacle(div_id_obstacle, updateRotation, updateTranslation);
```
