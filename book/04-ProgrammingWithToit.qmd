...
The code found in the [`MobileRoboticsCode`](https://github.com/anthonyjclark/MobileRoboticsCode) repository is setup so that you can use it as a set of libraries.
...
The Toit convention is to name your library file the same as its parent directory (eg, the `motors` directory contains a `motors.toit` file). You can then import the library using a relative path. For example, I import the `pinout` library in `motors.toit` with `import ..pinout`.

Here is an example of creating a new program using the libraries:
